...
| `OPENAI_API_KEY` | From platform.openai.com (transcription, call scoring, AI scripts) |
| `ANTHROPIC_API_KEY` | From console.anthropic.com (AI Agent autonomous follow-ups) |
| `ANTHROPIC_MODEL` | Optional — overrides the default agent model |
| `SENDGRID_API_KEY` | From SendGrid |
| `SENDGRID_FROM_EMAIL` | `braddockjones4@icloud.com` |
| `ENCRYPTION_KEY` | Auto-generated by Render — encrypts stored OAuth tokens (iCloud/Google) at rest |
| `GOOGLE_CLIENT_ID` | From console.cloud.google.com (Gmail blast, Google Contacts import) |
| `GOOGLE_CLIENT_SECRET` | From console.cloud.google.com |
| `BACKEND_URL` | This service's own public URL — used to build Twilio webhook URLs |
| `NODE_ENV` | `production` |

> Stripe billing (`STRIPE_SECRET_KEY`, `STRIPE_PRICE_*`, etc.) is not part of this deployment — billing routes are present in the code but unmounted (`server.ts`), per-client deployments don't use SaaS billing.
