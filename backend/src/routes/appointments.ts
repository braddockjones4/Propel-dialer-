...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const ownedContact = await prisma.contact.findFirst({ where: { id: contactId, userId } as any, select: { id: true } });
    ...
    if (sendSms !== false) {
      const { client: apptClient, creds: apptCreds } = await getTwilioClient(userId);
...
router.patch('/:id', async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id as string | undefined;
    const owns = await db.appointment.findFirst({ where: { id: req.params.id, contact: { userId } }, select: { id: true } });
    if (!owns) { res.status(404).json({ error: 'Appointment not found' }); return; }
