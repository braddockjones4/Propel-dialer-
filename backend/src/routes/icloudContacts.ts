...
async function importContacts(contacts: Record<string, string>[], noPhone = 0, userId?: string) {
  ...
  const existingCount = await prisma.contact.count({ where: { userId, phone: { in: phones } } });
  const scoped = contacts.map(c => ({ ...c, userId }));
  const result = await (prisma as any).contact.createMany({ data: scoped, skipDuplicates: true });
